...
def change_report(prev_rows: list[dict], new_rows: list[dict],
    ...
    위험한 것은 **규정의 현행 조문이 줄어드는 것**이다. 정상 개정에서는 구판이
    강등되는 동시에 신판이 들어와 현행 수가 유지된다. 줄었다면 신판을 못 받은
    것이고, 그대로 배포하면 그 규정에 「현행 근거 없음」이 된다.
    """
    ...
    return {
        ...
        # 검사는 사실상 꺼진 상태이며, 그 사실이 보고서에 남아야 한다.
        "current_source": "engine" if (prev_current is not None
                                       and new_current is not None) else "row_field",
        "distribution": {
            "record_type": {
