...
# ⚠️ 미배포 로컬 변경이 있다 (2026-09-07). 다음 갱신 배포에 **함께** 올라간다.
#   dedupe_records.py 3회 적용 — 합계 400행 제거. 로컬 17,139 / 운영 배포본 17,539.
#     f24b83e83578  별표 유출 18(「불가항력」×2) + 완전 중복 18(부칙 시행일)
#     84678139442e  「불가항력」 잔여 9(현행본 key 4213 포함) — 원문 별표 구간 판정
#     5ada414eb25f  연구비 지침 계약서 조항 336 + 정보시스템 지침 약관 조항 19
#     7820fc644856  연구비 지침 「해석」·「관할법원」 10 — 조항+서명란+별표 전문이
#                   한 레코드에 뭉친 것(14,429~16,263자). 같은 조문번호에 진짜
#                   조문이 따로 있어 지워도 답이 남는다.
#   합계 410행. 적재 기준 16,908 / 17,291. 별표(계약서·약관) 조항이 본칙으로
#   새어 나온 것을 걷어낸 것이라 원문은 전부 별표 레코드에 보존돼 있다.
# 다음 배포가 끝나면 이 주석을 지운다. 남아 있다는 것은 아직 안 올라갔다는 뜻이다.
